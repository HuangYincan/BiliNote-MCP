...
3. **至少一个 LLM 供应商已配置**（`list_providers` 查看；没有则 `add_provider`）。
4. 转写引擎二选一：
...
3. **`list_providers()`** —— 找一个启用的 LLM 供应商；没有就：
   `add_provider(name, api_key, base_url, type)` → `list_models(provider_id)`（实时拉取）或 `add_model(provider_id, model_name)`（手动添加）。
...
| 场景 | 操作 |
|------|------|
| 新增 LLM 供应商 | `add_provider(name="DeepSeek", api_key="sk-...", base_url="https://api.deepseek.com", type="deepseek")` |
...
| 切云端转写 | `set_transcriber("groq")`（需已有 id 为 `groq` 的供应商） |
...
| `generate_note` 报「需要 provider_id」 | 先 `list_providers` / `add_provider` |
