...
- **SKILL 并发流程修正（并行提交 + 轻量轮询）**：客户端与服务端都支持多任务并发（`BILINOTE_MAX_WORKERS=3`）—— 同一消息可并行提交多个 `generate_note`（各自返回 task_id）；但**多任务轮询必须用轻量 `get_task_status(task_id)` 快照轮询**，不要用阻塞的 `wait_for_note`（会卡住当前轮次、让对话像挂起）。提交前先告诉用户要并行提交哪些任务。README / docs/04 同步。
