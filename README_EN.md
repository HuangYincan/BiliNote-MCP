...
Mechanical formats use `export_transcript` (srt / vtt / json) — deterministic rendering (timestamp math), **no LLM**, returns `file://` paths. Creative formats (mindmap / flashcards / LaTeX / typst / user-custom templates) are generated by the **Agent from the MD draft + SKILL templates** (LaTeX ships academic / lecture / meeting_minutes / minimal styles).
