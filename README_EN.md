...
### Advanced: storage layout & cleanup

**One folder per task** `note_results/{task_id}/`: `raw/` (downloaded media) + `gen/` (transcript/note/frames/exports) + control files. **Global task index** lives in the SQLite `video_tasks` table (with semantic title/status/summary).

Cleanup (agent tools or setup ④ data management):

- **Inspect first**: `list_tasks()` enumerates all tasks (with titles) → `get_task_files(task_id)` shows a single task's files.
- **Per-task cleanup**: `cleanup_note(task_id, include_note=False)` — task-folder scoped; `include_note=False` deletes `raw/` + non-note items in `gen/` (**keeps the final note**), `include_note=True` deletes the whole task folder + index record.
- **Global cleanup (factory reset)**: `cleanup_all(include_config=False, include_models=False)` — empties `note_results/*`, `static/screenshots/*`, `logs/*` + the global index; **keeps** `config/` and `models/` by default.

Safety: task-folder scoped, `resolve()`-validated to stay inside the data directory (path-traversal safe); failures are skipped one-by-one and reported.

### Advanced: output formats (multi-format export)

After a note/transcript is produced, it can be exported to multiple formats:

- **Mechanical formats** (SRT/VTT/JSON): `export_transcript(task_id, formats=["srt","vtt","json"])` — deterministic rendering (timestamp math), **no LLM**, returns `file://` paths. If setup ③ configured "default export formats", these are auto-exported after each successful task.
- **Creative formats** (mindmap / flashcards / LaTeX / typst / user-custom templates): generated by the **Agent from the MD note draft**. LaTeX ships built-in style templates (`skills/bilinote/templates/latex/`: academic / lecture / meeting_minutes / minimal) — the user picks a style and the Agent generates `.tex` from the template (optional `xelatex` to compile PDF); user-provided templates are supported too. See the SKILL's `reference/output-formats.md`.

### Advanced: audio enhance

- **Platform coverage**: URLs outside the built-in 5 platforms get `platform:"generic"` → **yt-dlp generic extraction** (1800+ sites); only if that also fails (login wall / JS rendering) does the Agent take over.
- **Multi-file merge**: `merge_audio(files, out_dir?)` — FFmpeg concat merges recordings/meeting segments/local videos into one 16kHz mono wav, then transcribe.
- **Audio preprocessing** (setup ② / `transcriber preprocess on`): normalize to 16kHz mono + auto-chunk long audio (>1800s) with timestamp offset. **Off by default**; zero extra deps.
- **Speaker diarization** (setup ② / `transcriber diarization on`): `diarize_media(...)` labels speakers via pyannote. **Optional heavy dep**: `pyannote.audio` + torch + `HF_TOKEN` + model license; install hint returned when missing.
...
| `validate_url` | Detect which platform a video link belongs to; URLs outside the built-in 5 platforms return `{supported:true, platform:"generic"}` (yt-dlp generic extraction) |
...
| `list_tasks` | List all tasks (global index, with semantic title/status), for agents to enumerate & identify |
| `get_task_files` / `cleanup_note` / `cleanup_all` | Inspect a task's files / clean one task (keeps the final note by default) / global cleanup (factory reset; keeps config & models by default), see [storage layout & cleanup](#advanced-storage-layout--cleanup) |
| `export_transcript` | Export a task's transcript to **deterministic formats** (srt/vtt/json, no LLM), returns file paths; creative formats (mindmap/LaTeX etc.) are generated by the Agent from the note draft (see [output formats](#advanced-output-formats)) |
| `merge_audio` | Merge multiple audio/video files into one 16kHz mono wav (FFmpeg concat), then transcribe/summarize (see [audio enhance](#advanced-audio-enhance)) |
| `diarize_media` | Speaker diarization (pyannote, optional heavy dep — needs HF_TOKEN + model license), returns speaker turns |
